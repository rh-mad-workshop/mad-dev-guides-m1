...
The RHV environment not only has the PostgreSQL Database VM deployed but it also hosts another VM which runs the old customer application on top of Apache Tomcat.
...
[source,bash,subs="+attributes,macros+"]
----
Switched to branch 'patch-postgres'
Your branch is up to date with 'origin/patch-postgres'.
----
