...
As you have realized in the previous *assessment* lab, you need to analyze the legacy applications in terms of identifying the actual line of code for the modernization issues as well as estimating time and effort for the modernization project.
...
First, you need to _download_ or _git clone_ the `modern-app-dev` from your link:https://gitea.%SUBDOMAIN%/%USERID%/modern-app-dev[Gitea repository^]. 
...
[NOTE]
====
Please make sure to see the following success popup message on the right top. It probably will take a few seconds to complete uploading the binary file. 
...
You can now see exactly where the issue is located in the source code. But you are still using a web environment.
