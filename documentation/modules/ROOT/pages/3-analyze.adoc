...
[NOTE]
====
We'll analyze a prebuilt WAR file rather than the *customers-tomcat-legacy* directory because the MTA doesn't support selecting multiple root paths in the Git repository. The existing "customers" application depends on the external configuration that stores in the *config-utils* directory.
====
