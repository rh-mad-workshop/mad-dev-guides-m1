...
The goal of this lab is to assess the siloed current application portfolio and identify potential issues and risks to implement advanced cloud native architecture with the application modernization.
...
Open a new browser to access the https://tackle-tackle-%USERID%.apps.%SUBDOMAIN%[Tackle web console^].
...
First step will be to select the stakeholders (_Homer Simpson_) and Stakeholder groups (_Retail Management_) involved with the assessment. Leave the default values.
