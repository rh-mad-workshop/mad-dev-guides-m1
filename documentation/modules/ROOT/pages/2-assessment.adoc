...
To start the assessment of the existing applications we will use https://www.konveyor.io/tools/tackle[Tackle^] to assess the `Customers` service, a Java-based application running on _Apache Tomcat_ on Red Hat Virtualization.
...
**Tackle Application Inventory** is the tool which selects applications for assessment by https://github.com/konveyor/tackle-pathfinder[Pathfinder^] and analysis by https://github.com/windup/[Windup^]. It provides four main functions:
...
Go to `Application inventory` on the left menu and click on `pencil` (edit) icon for the customers inventory.
...
Read more about application modernization with Konveyor at https://Konveyor.io[Konveyor.io^].